...
function getTransporter() {
  ...
  _transporter = nodemailer.createTransport({
    ...
    auth: { user, pass },
    tls: {
      // Hosting compartido (cPanel) suele usar cert self-signed
...
async function send({ to, subject, html }) {
  ...
  if (!t) {
    ...
    console.warn('[email][FALLBACK] CORREO NO ENVIADO — SMTP sin configurar.');
    console.warn('[email][FALLBACK] Destinatario:', to);
    console.warn('[email][FALLBACK] Asunto:', subject);
    console.warn('[email][FALLBACK] Solución: establecer SMTP_HOST, SMTP_USER y SMTP_PASS en Railway.');
    console.warn('[email][FALLBACK] ═══════════════════════════════════════════');
    return { ok: true, fallback: true };
  }

  // SMTP_FROM es el email remitente verificado.
  // Con SendGrid: SMTP_USER="apikey" (no es email), por eso se usa SMTP_FROM como fallback.
