...
export const registrarInstructor = async (req, res) => {
  try {
    ...
    enviarEmailVerificacionInstructor({ email: user.email, nombre: user.nombre, confirmUrl })
      .then(r => {
        if (r?.fallback) console.warn('[auth][registrarInstructor] ⚠️  SMTP sin configurar. Correo de verificación NO enviado a:', user.email);
      })
  ...
  try {
    ...
    if (result) {
      ...
      enviarEmailRecuperacion({ email: result.user.email, nombre: result.user.nombre, resetUrl })
        .then(r => {
          if (r?.fallback) {
            console.warn('[auth][forgotPassword] ⚠️  SMTP sin configurar. Correo de recuperación NO enviado a:', result.user.email);
            console.warn('[auth][forgotPassword] Configura SMTP_HOST, SMTP_USER y SMTP_PASS en Railway para habilitar el envío.');
          }
        })
  ...
  try {
    ...
    if (result) {
      ...
      enviarEmailVerificacionInstructor({ email: result.user.email, nombre: result.user.nombre, confirmUrl })
        .then(r => {
          if (r?.fallback) console.warn('[auth][reenviar] ⚠️  SMTP sin configurar. Correo de confirmación NO reenviado a:', result.user.email);
        })
